    ...
    describe('deepCloneOnPath', () => {
        describe('patchValue is undefined', () => {
            it('deletes a property', () => {
                ...
                expect(deepCloneOnPath(originalState, ['a'], undefined)).toEqual({b: 2});
            ...
            it('ignores unknown state', () => {
                const originalState = {a: 1, b: 2};
                expect(deepCloneOnPath(originalState, ['c'], undefined)).toEqual({a: 1, b: 2});
                expect(originalState).toEqual({a: 1, b: 2});
            });

            it('ignores deep unknown state, variant 1', () => {
                const originalState = {a: 1, b: 2};
                expect(deepCloneOnPath(originalState, ['c', 'd'], undefined)).toEqual({a: 1, b: 2});
                expect(originalState).toEqual({a: 1, b: 2});
            });

            it('ignores deep unknown state, variant 2', () => {
                const originalState = {a: 1, b: 2, c: {d: 4}};
                expect(deepCloneOnPath(originalState, ['c', 'e'], undefined)).toEqual({a: 1, b: 2, c: {d: 4}});
        ...
        describe('patchValue a defined value', () => {
            it('updates property', () => {
                const originalState = {a: 1, b: 2};
                expect(deepCloneOnPath(originalState, ['a'], 11)).toEqual({a: 11, b: 2});
                expect(originalState).toEqual({a: 1, b: 2});
            });

            it('updates a deep property', () => {
                const originalState = {a: {c: 3}, b: 2};
                expect(deepCloneOnPath(originalState, ['a', 'c'], 'hello')).toEqual({a: {c: 'hello'}, b: 2});
                expect(originalState).toEqual({a: {c: 3}, b: 2});
            });
    ...
    describe('apply', () => {
        ...
        describe('batch', () => {
            ...
            it('applies a single action', () => {
                const state = {a: 1, b: 2};
                const newState = apply(state, {type: 'batch', actions: [{type: 'set', path: ['c'], value: 3}]});
